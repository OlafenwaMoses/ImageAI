...
def time_distributed_basic_2d(filters, stage=0, block=0, kernel_size=3, numerical_name=False, stride=None, freeze_bn=False):
    ...
    Usage:
        ...
        >>> blocks.time_distributed_basic_2d(64)  # doctest: +ELLIPSIS
        <function time_distributed_basic_2d.<locals>.f at ...>
    ...
    def f(x):
        y = keras.layers.TimeDistributed(keras.layers.ZeroPadding2D(
            padding=1), name="padding{}{}_branch2a".format(stage_char, block_char))(x)
        y = keras.layers.TimeDistributed(keras.layers.Conv2D(
            filters, kernel_size, strides=stride, use_bias=False, **parameters),
            name="res{}{}_branch2a".format(stage_char, block_char))(y)
        y = keras.layers.TimeDistributed(keras_resnet.layers.BatchNormalization(axis=axis, epsilon=1e-5, freeze=freeze_bn),
                                         name="bn{}{}_branch2a".format(stage_char, block_char))(y)
        y = keras.layers.TimeDistributed(keras.layers.Activation("relu"),
                                         name="res{}{}_branch2a_relu".format(stage_char, block_char))(y)

        y = keras.layers.TimeDistributed(keras.layers.ZeroPadding2D(padding=1),
                                         name="padding{}{}_branch2b".format(stage_char, block_char))(y)
        y = keras.layers.TimeDistributed(keras.layers.Conv2D(filters, kernel_size, use_bias=False, **parameters),
        ...
        if block == 0:
            ...
            shortcut = keras.layers.TimeDistributed(keras_resnet.layers.BatchNormalization(axis=axis, epsilon=1e-5, freeze=freeze_bn),
...
def time_distributed_bottleneck_2d(filters, stage=0, block=0, kernel_size=3, numerical_name=False, stride=None, freeze_bn=False):
    ...
    Usage:
        ...
        >>> from imageai.Detection.keras_resnet import blocks
        >>> blocks.time_distributed_bottleneck_2d(64)  # doctest: +ELLIPSIS
        <function time_distributed_bottleneck_2d.<locals>.f at ...>
    """
    if stride is None:
        stride = 1 if block != 0 or stage == 0 else 2
    ...
    def f(x):
        ...
        y = keras.layers.TimeDistributed(keras.layers.Conv2D(filters * 4, (1, 1), use_bias=False, **parameters),
        ...
        if block == 0:
            shortcut = keras.layers.TimeDistributed(keras.layers.Conv2D(filters * 4, (1, 1), strides=stride, use_bias=False, **parameters),
                                                    name="res{}{}_branch1".format(stage_char, block_char))(x)
            shortcut = keras.layers.TimeDistributed(keras.layers.BatchNormalization(axis=axis, epsilon=1e-5, trainable=(not freeze_bn)),
                                                    name="bn{}{}_branch1".format(stage_char, block_char))(shortcut)
        else:
            shortcut = x
