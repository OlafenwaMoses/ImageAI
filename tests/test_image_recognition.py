...
def test_recognition_model_squeezenet():
    ...
    predictor.loadModel()
    predictions, probabilities = predictor.predictImage(image_input=os.path.join(TEST_FOLDER, TEST_FOLDER, "data-images", "1.jpg"))
...
def test_recognition_model_squeezenet_array_input():
    ...
    predictor.setModelPath(os.path.join(TEST_FOLDER, "data-models", "squeezenet_weights_tf_dim_ordering_tf_kernels.h5"))
    predictor.loadModel()
    image_array = cv2.imread(os.path.join(TEST_FOLDER, TEST_FOLDER, "data-images", "1.jpg"))
